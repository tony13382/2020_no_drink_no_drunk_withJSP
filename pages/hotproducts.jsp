...
<%
sql="SELECT `GoodsName`, SUM(`Amount`) FROM `order_product` GROUP BY `GoodsName`";
ResultSet sum= con.createStatement().executeQuery(sql);

while(sum.next())
{
	sql="SELECT `GoodsName` FROM `sales` WHERE `GoodsName`= '"+sum.getString(1)+"'; ";
	ResultSet check= con.createStatement().executeQuery(sql);	
	if( !check.next() )
	{		
		sql = "insert into `sales`(`GoodsName`,`Amount`) values('"+sum.getString(1)+"' , '"+sum.getString(2)+"' );";
		con.createStatement().execute(sql);
	}
	else
	{				
		sql = "Update `sales` set `Amount` = '" +sum.getString(2) + "' Where `GoodsName` =  '"+sum.getString(1)+"' ";
		con.createStatement().execute(sql);
	}
}
%>
  ...
  <section>
    ...
    <div class="back_block">
      <div class="block_countainer">
        <!--1個產品元件Start-->
		<%  sql="SELECT * FROM `sales` ORDER BY `Amount` DESC LIMIT 3";
				    ResultSet hot= con.createStatement().executeQuery(sql);
					int rank = 0;
					 while(hot.next())
					{
					sql="SELECT * FROM `product` WHERE `GoodsName`='"+hot.getString(1)+"' ";
        ...
        <div class="row block_bigrow">
          <div class="col-2 d-flex flex-column">
            <img width="100%" class="d-flex align-items-center" src="../assets/images/page_element/king<%=rank%>.png">
          </div>
          <div>
            <div class="block_img ">
              <a href="item.jsp?itemid=<%=rsr.getString(11)%>">
                                <img width="100%" src="../assets/images/product_img/<%=rsr.getString(10)%>">
                            </a>
            </div>
          </div>
          <div class="col-5">
		  <br>
		  <br>
            <h3 class="favroite_title"><%=rsr.getString(1)%></h3>
            <div class="favroite_info">
              <p>
               
              </P>
			  <br>
		  <br><br>
              <div class="favroite_toinfo_block">
                <h3 class="favroite_title favroite_toinfo_title">商品詳情</h3>
                <a href="item.jsp?itemid=<%=rsr.getString(11)%>">
                  <img src="../assets/images/page_element/click.png" height="100px">
                </a>
              </div>
            </div>
          </div>
